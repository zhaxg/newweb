...
const API_ROOT = opt(
  "api-root",
  join(dirname(resolve(fileURLToPath(import.meta.url))), "..", "..", "..", "..", "src", "api", "mes4ddh"),
);
...
function findServiceFile(root, name) {
  ...
  for (let n = 0; stack.length && n < 40000; n++) {
    ...
    let ent;
    try {
      ent = readdirSync(dir, { withFileTypes: true });
    } catch {
      continue;
    }
    for (const e of ent) {
...
function existingApis() {
  ...
  for (const f of files) {
    ...
    let src;
    try {
      src = readFileSync(join(API_ROOT, f), "utf8");
    } catch {
      continue;
    }
...
function reportCalls(designerPath) {
  ...
  for (const raw of lines) {
    ...
    for (const m of line.matchAll(
      /(\w*[Bb]indingSource)?\s*\.?\s*GetTrackingList\s*<\s*([\w.]+)\s*>\s*\(\s*\)\s*\.\s*ToSaveChangesData/g,
    )) {
  ...
  if (!calls.length) console.log("  服务调用: 无（.cs 里没有 Svc<I*AppService>.Proxy / Proxy<I*AppService>() 调用）");
    ...
    for (const c of calls) {
      ...
      const state = !apis.has(api)
        ? "需补(整个 Api 对象)"
        : apis.get(api).has(camel(c.method))
          ? "已生成"
          : "需补(缺该方法)";
      console.log(
  ...
  for (const t of tuniq) {
    console.log(
      `    ${(t.fn || "保存").padEnd(28)} ${t.src || "?"}.GetTrackingList<${t.entity}>().ToSaveChangesData()  [跟踪]`,
    );
    console.log(
      `  ${" ".repeat(28)}→ new TrackableList<${t.entity}>(rows) + crudAppService.SaveList(list, "${t.entity}")`,
    );
  }
...
function parse(src) {
  ...
  for (const m of src.matchAll(
    /\b(?:private|internal|protected|public)\s+(?:global::)?([\w.:]+(?:<[^>]+>)?)\s+(\w+)\s*;/g,
  )) {
  ...
  for (const m of src.matchAll(
    /(?:this\.)?(\w+)\.(Columns|TabPages|ItemLinks|Views|Panels)\.AddRange\([^)]*\{([^}]*)\}\s*\)/g,
  )) {
...
function parseExtras(src) {
  ...
  for (const m of src.matchAll(
    /(?:this\.)?(\w+)\.DataBindings\.Add\(\s*new (?:global::)?[\w.]*Binding\(\s*"(\w+)"\s*,\s*(?:this\.)?([\w.]+)\s*,\s*"([^"]*)"/g,
  )) {
  ...
  for (const m of src.matchAll(/(?:this\.)?(\w+)\.Properties\.Items(?:\.AddRange)?\([^;]*?\{([^}]*)\}/g)) {
    const list = [...m[2].matchAll(/\(\s*"([^"]*)"\s*(?:,\s*([^,()]+?))?\s*[,)]/g)].map((x) => ({
      text: x[1],
      value: (x[2] ?? "").trim(),
    }));
...
function report(path, parsed) {
  ...
  const maskOf = (col) => {
    ...
    if (!m) return "";
    return [m.EditMask && `mask=${m.EditMask}`, m.MaskType && m.MaskType !== "None" && `maskType=${m.MaskType}`]
      .filter(Boolean)
      .join(" ");
  };
  const itemsOf = (col) => {
  ...
  for (const sp of decls) {
    ...
    console.log(
      `\n分栏 ${sp[0]}: ${horiz === "false" ? "上下" : "左右"}  SplitterPosition=${P(sp[0], "SplitterPosition") ?? "-"}  ` +
        `Panel1=${byKind("Panel1")
          .filter((a) => a.owner === sp[0])
          .map((a) => a.items[0])
          .join("/")}  ` +
        `Panel2=${byKind("Panel2")
          .filter((a) => a.owner === sp[0])
          .map((a) => a.items[0])
          .join("/")}`,
    );
  ...
  btns.sort((a, b) => {
    const ia = btnOrder.indexOf(a[0]),
      ib = btnOrder.indexOf(b[0]);
    if (ia >= 0 || ib >= 0) return (ia < 0 ? Infinity : ia) - (ib < 0 ? Infinity : ib);
    const ca = addIdx.get(a[0]),
      cb = addIdx.get(b[0]);
  ...
  if (shown.length) {
    ...
    shown.forEach(([n], i) =>
      console.log(
        `  ${i}  ${(P(n, "Text") ?? P(n, "Caption") ?? "?").padEnd(12)} [${n}]  容器=${addOwner.get(n) ?? "-"}  Visible=${P(n, "Visible") ?? "true"}`,
      ),
    );
  }
  ...
  for (const [vn] of views) {
    ...
    const cols = declared.length
      ? declared
      : [...decls].filter(([, d]) => LABELS.gridColumn(short(d.type))).map(([n]) => n);
    const rows = cols
      .map((c) => ({
        c,
        f: P(c, "FieldName"),
    ...
    const idx = (v) =>
      v !== undefined && v !== "-1" && Number.isFinite(Number(v)) && Number(v) >= 0 ? Number(v) : null;
    ...
    for (const r of vis)
      console.log(
        `  ${String(r.vi).padStart(2)}  ${(r.f ?? "?").padEnd(24)} ${cn(r, entity)}  w=${r.w ?? "-"}${r.fmt ? `  fmt=${r.fmt}` : ""}${extrasOf(r.c) ? "  " + extrasOf(r.c) : ""}`,
      );
    if (amb.length)
      console.log(
        "  待确认(Visible=true 但未排 VisibleIndex，需回读 Designer 或看原画面): " +
          amb.map((r) => `${r.f ?? r.c}`).join("  "),
      );
    if (hid.length)
      console.log(
        "  hide: " +
          hid
            .map(
              (r) =>
                `${r.f ?? r.c}=${labels.byEntity.get(entity)?.get(r.f) ?? labels.global.get(r.f) ?? r.cap ?? "?"}${r.vis === "false" ? "(Visible=false)" : ""}`,
            )
            .join("  "),
      );
  }
  ...
  if (!FLAG.quiet) {
    ...
    const inputs = [...decls]
      .filter(([, d]) => LABELS.input(short(d.type)) && !/^RepositoryItem/.test(short(d.type)))
      .map(([n]) => n);
    ...
    if (groups.size || captioned.length) {
      ...
      for (const [host, list] of groups) {
        ...
        for (const n of list) {
          ...
          const ex = [b && `bind=${b}`, P(n, "EditName") && `editName=${P(n, "EditName")}`, extrasOf(n)]
            .filter(Boolean)
            .join("  ");
    ...
    if (repUsed.length) {
      ...
      for (const n of repUsed)
        console.log(
          `  ${n.padEnd(26)} ${short(decls.get(n).type).padEnd(24)} ${[maskOf(n), itemsOf(n)].filter(Boolean).join("  ")}`,
        );
    }
  }
}
...
function loadLabels(root) {
  ...
  for (const f of files) {
    ...
    for (const m of s.matchAll(
      /\[(?:LDisplay|Description|DisplayName)\("([^"]+)"\)\][\s\S]{0,240}?public\s+[\w<>[\],.?\s]+?\s+(\w+)\s*(?:\{|=>)/g,
    )) {
