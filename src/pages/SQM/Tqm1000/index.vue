...
/** 对应 FrmTqm1000（内控钢种）：DDH.Winforms.SQM.Forms.Tmptq.FrmTqm1000
 *  画面迁移，逻辑不迁移到
 *  2026-09-22 已完成精修 */
...
const colDefs: ColDef[] = [
  ...
  { field: "CSgCodeNk", headerName: "内控钢种", width: 100 },
  { field: "NThickMin", headerName: "最小厚度", width: 90 },
  { field: "NThickSwitch", headerName: "厚度区间", width: 90 },
  { field: "NThickMax", headerName: "最大厚度", width: 90 },
  { field: "CSpecialMarkGy", headerName: "特殊标记工艺", width: 120 },
  { field: "CNature", headerName: "性质", width: 80 },
...
<template>
  <div class="flex min-h-0 flex-1 flex-col">
    ...
    <div class="shrink-0 border-b border-border/60 px-3 py-2">
      <div class="grid grid-cols-6 items-center gap-x-3 gap-y-1.5">
        <div class="flex min-w-0 items-center gap-1.5">
          ...
          <Select v-model="input.lineCode" :options="[]" show-clear class="min-w-0 flex-1" />
        </div>
        <div class="flex min-w-0 items-center gap-1.5">
          <label class="w-16 shrink-0 text-xs text-muted-foreground">性能</label>
          <Select v-model="input.keepPerf" :options="[]" show-clear class="min-w-0 flex-1" />
        </div>
        <div class="flex min-w-0 items-center gap-1.5">
          <label class="w-16 shrink-0 text-xs text-muted-foreground">关键字</label>
          <InputText v-model="input.keyword" class="min-w-0 flex-1" />
        </div>
      </div>
    </div>
    ...
    <div class="flex h-9 shrink-0 items-center gap-1 border-b border-border/60 px-2">
      ...
      <Button text class="shrink-0 whitespace-nowrap" @click="onBtn0"><IconDeviceFloppy class="h-3 w-3" />保存</Button>
      ...
      <span class="ml-auto text-xs text-muted-foreground">内控钢种（{{ rows.length }}）</span>
    </div>
