 ...
 *  已接入：tql1070Api.queryStorage / queryTql1070（原 btnQueryRecord→LoadRecord 用 QueryTql1070；queryRecord 在 swagger 备用）/ setDisable；库区下拉 tyd1000Api.queryRoom（原 StoreFormatter→ITyd1000AppService.QueryRoom）
 *  待接入：探伤判定二级弹窗 FrmQZ6007（未勾选校验「请勾选需要判定的材料后在操作」照抄后 toast）
 *  菜单参数：cQueryString JSON {CStoreCode,NProType} 经 useMenuQuery 下发（库区初值 + 库存分类；缺省回落 ZG01-01/坯料）
 *  结构：上=库存 ucStorage1，下=stackPanel2（记录查询条）+ 判定记录（原 splitContainerControl1 上下 SplitterPosition 508）
 *  库存表：NProType=P→InitSlabColumns / C→InitP4Columns 置前可见，其余 Designer 列 hide:true
 *  记录表：extract 25 可见 + 17 hide；Selected → 行选择勾选列
 *  偏差：原 SetCodeFormatterAsync 字典翻译未迁，web 显示原值；已内联翻译产线/库存分类/表检结果 */
...
import { tql1070Api, type TimeRange } from "@/api/mes4ddh/lims.swagger";
import type { Tyd2000Dto, Tyd1000 } from "@/api/mes4ddh/syd.swagger";
import { tyd1000Api } from "@/api/mes4ddh/syd.swagger";
...
/* ---------- 库区（原 cmbStoreCode：StoreFormatter→tyd1000Api.queryRoom，初值 qs.CStoreCode） ---------- */
const storeOptions = ref<{ label: string; value: string }[]>([]);
const storageQuery = reactive({
  storeCode: qsStoreCode,
  stove: "",
  pieceNo: "",
...
async function loadStores() {
  try {
    const rooms = (await tyd1000Api.queryRoom()) ?? [];
    storeOptions.value = (rooms as Tyd1000[])
      .filter((r) => r.cStoreCode)
      .map((r) => ({ label: String(r.cStoreDes || r.cStoreCode), value: String(r.cStoreCode) }));
    if (storageQuery.storeCode && !storeOptions.value.some((o) => o.value === storageQuery.storeCode)) {
      storeOptions.value.unshift({ label: storageQuery.storeCode, value: storageQuery.storeCode });
    }
  } catch {
    /* 拦截层已 toast */
  }
...
/* ---------- 判定记录表列（原 gvTql1070Record 按 VisibleIndex；Selected 由行选择承担） ---------- */
const recordColDefs: ColDef[] = [
  ...
  { field: "cSgStd", headerName: "执行标准", width: 112 },
  { field: "cSpec", headerName: "规格", width: 86 },
  { field: "cSurfaceUser", headerName: "判定人", width: 99 },
  { field: "dSurfaceTime", headerName: "判定时间", width: 112 },
  { field: "cDetectResultCode", headerName: "探伤判定结果", width: 138 },
  { field: "cDetectDefectLevel", headerName: "探伤等级", width: 112 },
  { field: "cDefectDefectCode", headerName: "探伤判定缺陷代码", width: 164 },
  { field: "cDefectDefectMark", headerName: "探伤判定缺陷描述", width: 164 },
  { field: "cShiftNo", headerName: "判定班次", width: 112 },
  { field: "cGroupNo", headerName: "判定班组", width: 112 },
  ...
  { field: "cSurfaceResult", headerName: "表检结果", width: 112, hide: true },
  { field: "cSurfaceDefectCode", headerName: "表面缺陷代码", width: 138, hide: true },
  { field: "cSurfaceDefectPosition", headerName: "表面缺陷位置", width: 138, hide: true },
  { field: "cSurfaceDesc", headerName: "表检描述", width: 112, hide: true },
  { field: "nSurfaceThick1", headerName: "尺寸厚1", width: 112, hide: true },
  { field: "nSurfaceThick2", headerName: "尺寸厚2", width: 112, hide: true },
  { field: "nSurfaceThick3", headerName: "尺寸厚3", width: 112, hide: true },
  { field: "nSurfaceLen", headerName: "尺寸长", width: 99, hide: true },
  { field: "nSurfaceWidth", headerName: "尺寸宽", width: 99, hide: true },
...
async function onQueryStorage() {
  ...
  try {
    storageRows.value =
      (await tql1070Api.queryStorage({
        storeCode: storageQuery.storeCode,
...
function onDetectJudge() {
  ...
  // 原 new FrmQZ6007(tyds).ShowDialog()：二级弹窗待接入（见来源注释）
  toast("探伤判定弹窗（FrmQZ6007）待接入", 2000, "warn");
}
...
function onDisable() {
  ...
  askConfirm("请确定作废判定记录？作废不影响现有判定结果", async () => {
    try {
      await tql1070Api.setDisable(ids);
    } catch {
...
<template>
  <div class="flex min-h-0 flex-1 flex-col">
    <!-- 查询条件（原 dataLayoutControl2 Dock=Top：库区/炉号/件次号/产出时间） -->
    ...
    <div class="flex h-9 shrink-0 items-center gap-1 border-b border-border/60 px-2">
      ...
      <span class="ml-auto text-xs text-muted-foreground">精整库探伤判定</span>
    </div>

    <!-- 上下分栏（原 splitContainerControl1 上下 SplitterPosition 508：上=库存 ucStorage1，下=stackPanel2+判定记录） -->
