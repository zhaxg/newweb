...
<template>
  <div class="flex min-h-0 flex-1 flex-col">
    ...
    <div class="flex h-9 shrink-0 flex-wrap items-center gap-2 border-b border-border/60 px-2">
      ...
      <Button text size="small" class="shrink-0 whitespace-nowrap" @click="onReset" label="重置">
        <template #icon>
    ...
    <div class="min-h-0 flex-1 overflow-hidden">
      <AgGridVue class="erp-ag-grid h-full w-full" :theme="theme" :column-defs="columnDefs"
        :default-col-def="erpDefaultColDef" :row-data="filtered" :get-row-id="getRowId" :row-selection="'single'"
...
<style scoped>
.erp-ag-grid :deep(.audit-result-ok) {
  color: var(--success);
  font-weight: 500;
}

.erp-ag-grid :deep(.audit-result-fail) {
