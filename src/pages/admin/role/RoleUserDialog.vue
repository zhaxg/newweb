...
const leftUsers = computed(() => {
  ...
  return users.value.filter(
    (u) =>
      !u.marked &&
      (!kw || (u.userName ?? "").toLowerCase().includes(kw) || (u.userId ?? "").toLowerCase().includes(kw)),
  );
});
...
<template>
  <Dialog
    :visible="open"
    modal
    :header="`角色[${role?.cRoleName}]对应的用户列表`"
  ...
  >
    <div class="flex min-w-0 gap-3">
      ...
      <div class="flex w-[42%] min-w-0 flex-col rounded-md border border-border/60">
        <div class="flex h-8 shrink-0 items-center border-b border-border/60 px-2 text-xs font-medium">
          所有用户列表
        </div>
        ...
        <div
          class="flex h-7 shrink-0 items-center border-b border-border bg-[rgb(239,239,239)] text-xs font-medium dark:bg-[rgb(32,32,34)]"
        >
      ...
      <div class="flex min-w-0 flex-1 flex-col rounded-md border border-border/60">
        ...
        <div class="min-h-0 max-h-[46vh] flex-1 overflow-y-auto">
          ...
          <div v-if="rightUsers.length === 0" class="px-2 py-6 text-center text-xs text-muted-foreground">
            该角色暂无成员，双击左侧用户加入
          </div>
        </div>
      </div>
