...
const FAVORITES = [
  "LayoutDashboard", "ShoppingCart", "Package", "PackageImport", "PackageExport", "Users", "UserSearch",
  "Building", "BuildingWarehouse", "ShieldCheck", "ListTree", "Braces", "Note", "FileText", "Settings",
  "Wallet", "TrendingUp", "Packages", "Truck", "Clock", "Sparkles", "AdjustmentsHorizontal", "Coin",
  "Cash", "Receipt", "Table", "ChartPie", "ChartBar", "ChartLine", "Search", "Refresh", "Folder",
  "Archive", "ClipboardList", "ClipboardCheck", "Scale", "Trophy", "Forklift", "ArrowsLeftRight", "Home",
];
...
const allMatches = computed(() =>
  query.value.trim() ? filterTablerIcons(query.value) : DEFAULT_LIST,
);
  ...
  <div class="flex min-w-0 items-stretch gap-1">
    ...
    <Popover ref="popover" :dismissable="true" class="w-[520px]" @hide="hideTip">
      <div class="flex flex-col gap-2 p-1">
        <div class="relative">
          <InputText v-model="query" ref="searchInput" placeholder="搜索图标（如 chart-pie / building / clock）"
        ...
        <div class="grid max-h-[260px] grid-cols-9 gap-1 overflow-y-auto" @scroll.passive="onScroll">
          <button v-for="name in visible" :key="name" type="button"
            ...
            @mouseenter="showTip($event, name)" @mousemove="showTip($event, name)" @mouseleave="hideTip"
            @click="pick(name)">
