...
{ field: "CStove", headerName: "虚拟炉号", width: 80 },
      ...
      { field: "CBeforeChangeId", headerName: "炉次生产改钢种前id 改钢种后新增加的数据记录的之前的数据id", width: 220, hide: true },
      { field: "CAfterChangeId", headerName: "炉次生产改钢种后id 改钢种后原数据记录的新增加的数据的id", width: 220, hide: true },
...
async function onQuery() {
  querying.value = true;
  try {
    const list = (await frmMS2100ZLVirtualStoveNoApi.getVirtualStoveNoDatas({
...
<template>
  <div class="flex min-h-0 flex-1 flex-col">
    <div class="flex h-9 shrink-0 items-center gap-2 border-b border-border/60 px-2">
      ...
      <Select v-model="q.machineCode" :options="machineOptions" option-label="label" option-value="value" class="w-32 shrink-0" />
