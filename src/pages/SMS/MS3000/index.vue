    ...
    for (const item of kv) {
      ...
      cfCols.value.push({
        ...
        valueGetter: (p) => {
          ...
          const tr = (row.TestResults ?? row.testResults) as
            | { CElmCode?: string; cElmCode?: string; NValue?: unknown; nValue?: unknown }[]
            | undefined;
...
function onOutputReady(e: GridReadyEvent) {
  outputApi.value = e.api;
}
...
<template>
  <div class="flex min-h-0 flex-1 flex-col">
    ...
    <div class="flex h-9 shrink-0 items-center gap-2 border-b border-border/60 px-2">
      ...
      <Button variant="outlined" class="shrink-0 whitespace-nowrap" :loading="saving" @click="onGSReback"
        >钢水回炉</Button
      >
      <Button variant="outlined" class="shrink-0 whitespace-nowrap" :loading="saving" @click="onGSRebackCancel"
        >取消回炉</Button
      >
    </div>
            ...
            <TabPanel value="output" class="h-full">
              <div class="h-full w-full">
                <AgGridVue
                  ...
                  :locale-text="AG_GRID_LOCALE_CN"
                  :default-col-def="hmxDefaultColDef"
                  :column-defs="outputCols"
                  :row-data="outputRows"
                  :pagination="false"
                  :loading="childLoading"
                  @grid-ready="onOutputReady"
                  @first-data-rendered="autoSizeOnFirstData"
                />
              </div>
            </TabPanel>
