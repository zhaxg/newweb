...
const outCols = ref<ColDef[]>(bridge([
...
function onOutReady(e: GridReadyEvent) { outApi.value = e.api; }
...
function updateSizeInfo() {
  const row = focusedPlan.value;
  if (!row) {
    f.nThick = 263; f.nWth = 2230; f.nLen = 6000; f.proRemark = "";
    return;
  }
...
async function onRefresh() {
  try { await loadOut(); } catch { /* 拦截层已 toast */ }
}

async function onAdd() {
  const row = focusedPlan.value;
  if (!row) return;
  if (!f.liu) { toast("禁止操作，流号不得为空！", 2500, "warn"); return; }
  if (lineCode === "LG01" && !(f.lenWgt > 0)) { toast("禁止操作，米单重不得小于或等于0！", 2500, "warn"); return; }
...
async function onMatch(isAllow: boolean) {
  ...
  if (storages.length === 0) { toast("请选择未匹配的材料", 2500, "warn"); return; }
  const planItem = focusedOrder.value;
  if (!planItem) { toast("请选择计划后再操作", 2500, "warn"); return; }
...
async function onCancelMatch() {
  const storages = outs.value.filter((r: any) => (r.selected === true || r.Selected === true) && (r.cOrderNo || r.COrderNo));
  if (storages.length === 0) { toast("请选择已匹配的材料", 2500, "warn"); return; }
  ...
  if (!reason.trim()) { toast("请选择取消原因", 2000, "warn"); return; }
  try {
...
async function onSetExp() {
  ...
  if (storages.length === 0) { toast("请选择后再操作", 2500, "warn"); return; }
  if (storages.some((r: any) => ["ConsumeLocked", "Consume"].includes(String(r.nStatus ?? r.NStatus ?? "")))) {
    toast("已投料生产，不允许操作", 2500, "warn");
    return;
  }
  ...
  if (v == null) return;
  if (!v.trim()) { toast("请选择异常坯类型后再确认", 2500, "warn"); return; }
...
async function onUpdatePiece() {
  const storages = selectedOuts(false);
  if (storages.length === 0) { toast("请选择要修改的的材料", 2500, "warn"); return; }
...
async function onUpdateSize() {
  const sel = outs.value.filter((r: any) => r.selected === true || r.Selected === true);
  if (sel.length === 0) { toast("请选择要修改的的材料", 2500, "warn"); return; }
...
<template>
  <div class="flex min-h-0 flex-1 flex-col">
    ...
    <div class="flex h-9 shrink-0 items-center gap-1 overflow-x-auto border-b border-border/60 px-2">
      ...
      <Select v-model="q.status" :options="statusOptions" option-label="label" option-value="value" show-clear class="w-24 shrink-0" />
        ...
        <Splitter layout="horizontal" class="h-full min-h-0">
          <SplitterPanel :size="58" :minSize="30" class="flex min-h-0 flex-col overflow-hidden">
            ...
            <div class="flex h-9 shrink-0 items-center gap-2 overflow-x-auto border-b border-border/60 px-2">
              ...
              <Select v-model="f.liu" :options="liuOptions" option-label="label" option-value="value" class="w-24 shrink-0" />
              ...
              <template v-if="isLg01">
                ...
                <Select v-model="f.spec" :options="specOptions" option-label="label" option-value="value" class="w-28 shrink-0" />
              ...
              <Select v-model="f.sgCodeSimp" :options="sgCodeOptions" option-label="label" option-value="value" show-clear editable class="w-32 shrink-0" />
            </div>
            <!-- 头/尾/异常坯设置（原 stackPanel2 + dropDownButton1） -->
            <div class="flex h-9 shrink-0 items-center gap-1 border-b border-border/60 px-2">
              <Button text class="shrink-0 whitespace-nowrap" @click="placeholder('头/尾/异常坯设置')">头/尾/异常坯设置</Button>
            ...
            <div class="min-h-0 flex-1 overflow-hidden">
              <AgGridVue class="hmx-ag-grid h-full w-full" :theme="theme" :locale-text="AG_GRID_LOCALE_CN"
                :default-col-def="hmxDefaultColDef" :column-defs="outCols" :row-data="outs"
                :pagination="false"
                :row-selection="{ mode: 'multiRow', checkboxes: true, headerCheckbox: true, enableClickSelection: true, enableSelectionWithoutKeys: true }"
                @grid-ready="onOutReady" @first-data-rendered="autoSizeOnFirstData" />
            </div>
          </SplitterPanel>
          ...
          <SplitterPanel :size="42" :minSize="25" class="flex min-h-0 flex-col overflow-hidden">
            <Splitter layout="vertical" class="min-h-0 flex-1">
              <SplitterPanel :size="55" :minSize="25" class="flex min-h-0 flex-col overflow-hidden">
                ...
                <div class="min-h-0 flex-1 overflow-hidden">
                  <AgGridVue class="hmx-ag-grid h-full w-full" :theme="theme" :locale-text="AG_GRID_LOCALE_CN"
                    ...
                    @grid-ready="onPlanReady" @first-data-rendered="autoSizeOnFirstData" @row-clicked="onPlanClick" />
                </div>
              </SplitterPanel>
              <SplitterPanel :size="45" :minSize="20" class="flex min-h-0 flex-col overflow-hidden">
                ...
                <div class="min-h-0 flex-1 overflow-hidden">
                  <AgGridVue class="hmx-ag-grid h-full w-full" :theme="theme" :locale-text="AG_GRID_LOCALE_CN"
                    :default-col-def="hmxDefaultColDef" :column-defs="orderCols" :row-data="orders"
                    :pagination="false"
                    :row-selection="{ mode: 'singleRow', checkboxes: false, enableClickSelection: true }"
                    @grid-ready="onOrderReady" @first-data-rendered="autoSizeOnFirstData"
