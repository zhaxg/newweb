...
/** 对应 FrmMS2230_LZ（连铸台账报表 3 菜单 LG02 共用 cQueryString={LineCode,MachineCode}）：DDH.Winforms.SMS.Forms.FrmMS2230_LZ
 *  已接入：crudAppService.SaveList("FrmMS2230_LZViewDto")（btnSave_Click：GetTrackingList→ToSaveChangesData；
 *         保存按钮门控 = 查询后任一行 IsEnableEditAndSaveDatas；本页网格可编辑恒为 true——照抄 .cs
 *         btnQuery_Click_Real 里 Editable=true（check 判断被注释掉的原样））
...
// 可编辑恒 true（照抄 .cs）；系统/内键列只读；班次/班组列下拉照列头文案
...
async function onSave() {
  ...
  saving.value = true;
  try {
    await crudAppService.SaveList(trackList.value, "FrmMS2230_LZViewDto");
...
function onAutoRefresh(val: boolean) {
  if (val && rangeDays() > 1) {
    toast("全选时时间范围禁止超过一天，请调整时间后重试！", 2500, "warn");
...
function onShowChart() {
  ...
  const bad = list
    ...
    .map((r) => String(r.CStoveNo ?? ""));
  if (bad.length) {
    toast(
      `开浇停浇时间不得为空且停浇时间不得早于开浇时间！\r\n请检查以下炉号信息：\r\n${bad.join(",\r\n")}`,
      4000,
      "warn",
    );
    return;
...
<template>
  <div class="flex min-h-0 flex-1 flex-col">
    <!-- 工具栏（原 stackPanel1：产线/机台/日期(UCTimeRange)/炉次号/查询/保存/查看曲线图/全选） -->
    <div class="flex h-9 shrink-0 items-center gap-2 border-b border-border/60 px-2">
      ...
      <Checkbox v-model="autoRefresh" binary input-id="chkAutoRefresh" @update:model-value="onAutoRefresh" />
      <label for="chkAutoRefresh" class="shrink-0 text-xs text-muted-foreground">全选</label>
    </div>
