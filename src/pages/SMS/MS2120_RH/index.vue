...
/** 对应 FrmMS2120_RH（RH 真空作业，多菜单共享 cQueryString={LineCode,MachineCode}）：DDH.Winforms.SMS.Forms.FrmMS2120_RH
 *  已接入：frmMS2120RHFullApi.queryRoutePlans / checkStoveArrived+stoveArrived / checkCancelStoveArrived+cancelStoveArrived / checkStoveLeave+stoveLeave / getTms2013RHData+saveTms2013RHData / frmMS2100XMessageApi.checkIsNeedShowDialog / uCMSStoveInputInfoApi.getStoveInputInfo+saveStoveInputInfo / uCGYYDApi.getGYYDInfo / uCCustomPLCPointShowInfoApi.query
...
const formFields = [
  ...
  { k: "dZb", t: "座包时间", m: "d" },
  { k: "dDb", t: "吊包时间", m: "d" },
  { k: "nProUsedTime", t: "冶炼时长秒", m: "i" },
  { k: "nElecTime", t: "通电时间秒", m: "g" },
  { k: "cTempDown", t: "座包温度" },
  { k: "cTemp1", t: "过程温度1" },
  { k: "cTemp2", t: "过程温度2" },
  { k: "cTempUp", t: "吊包温度" },
  ...
  { k: "nRc", t: "软吹时长秒", m: "i" },
  { k: "nTempNum", t: "测温次数", m: "i" },
  { k: "cSlagCond", t: "进站渣况" },
  { k: "cTempTransf", t: "变压器油温" },
  ...
  { k: "d100PaBeg", t: "100Pa开始时刻", m: "d" },
  { k: "d100PaEnd", t: "100Pa结束时刻", m: "d" },
  { k: "cPotSituation", t: "钢包情况透气性" },
  { k: "nPotJk", t: "钢包净空(mm)", m: "g" },
  { k: "nPotZh", t: "钢包渣厚(mm)", m: "g" },
  { k: "dProBeg", t: "生产开始时间", m: "d" },
  { k: "nGetZkUsedTime", t: "抽真空时间秒", m: "i" },
  { k: "nZkUsedTime", t: "深真空时间秒", m: "i" },
  { k: "nZkCycleUsedTime", t: "净循环时间秒", m: "i" },
  { k: "dStaIn", t: "进站时间", m: "d" },
  { k: "dStaOut", t: "出站时间", m: "d" },
  { k: "dProZkDoneEnd", t: "生产真空结束时间", m: "d" },
  { k: "nInUsedTime", t: "处理时间秒", m: "i" },
  { k: "nO2UsedTime", t: "吹氧时长", m: "i" },
  { k: "nO2Used", t: "耗氧量m³", m: "g" },
  { k: "nInO2Press", t: "进站氧气压力(ppm)", m: "g" },
  { k: "cEndO", t: "终点氧", m: "g" },
  { k: "cTempIn", t: "进站温度℃" },
  { k: "cTempOut", t: "出站温度℃" },
  { k: "cZkMax", t: "最高真空度" },
  { k: "cTempZk", t: "破空温度℃" },
  { k: "nAlRepre", t: "铝回收率", m: "g" },
  { k: "cCrossSection", t: "断面宽度（mm×mm)" },
  { k: "cBackup", t: "备注", m: "m" },
];
...
const inputDetailCols = ref<ColDef[]>(
  bridge([
...
function onInputReady(e: GridReadyEvent) {
  inputApi.value = e.api;
}
function onInputDetailReady(e: GridReadyEvent) {
  inputDetailApi.value = e.api;
}
...
async function loadBof(mandatory = true) {
  if (!mandatory && !autoRefreshRh.value) return;
  const row = focused.value;
  try {
    if (!row) {
    ...
    const data = (await frmMS2120RHFullApi.getTms2013RHData(id)) ?? {};
    Object.keys(bof).forEach((k) => delete bof[k]);
...
async function onQuery(mandatory = true) {
  ...
  querying.value = true;
  try {
    plans.value = (await frmMS2120RHFullApi.queryRoutePlans(dto)) ?? [];
...
async function onArrival() {
  ...
  if (!row) return;
  try {
    await frmMS2120RHFullApi.checkStoveArrived(tms2010Of(row));
    if (!window.confirm(`确定对炉次号为‘${row.tms2000_cStove}’的炉次执行炉次到站操作？`)) return;
    await frmMS2120RHFullApi.stoveArrived({ tms2010: tms2010Of(row), shiftInfoDto: null, operationModeEnum: "Custom" });
    await onQuery(true);
    await showLastProcException();
...
async function onArrivalCancel() {
  ...
  if (!row) return;
  try {
    await frmMS2120RHFullApi.checkCancelStoveArrived(tms2010Of(row));
    if (!window.confirm(`确定对炉次号为‘${row.tms2000_cStove}’的炉次执行取消炉次到站操作？`)) return;
    await frmMS2120RHFullApi.cancelStoveArrived(tms2010Of(row));
    await onQuery(true);
...
async function onOut() {
  ...
  if (!row) return;
  try {
    await frmMS2120RHFullApi.checkStoveLeave(tms2010Of(row));
    if (!window.confirm(`确定对炉次号为‘${row.tms2000_cStove}’的炉次执行炉次出站操作？`)) return;
    await frmMS2120RHFullApi.stoveLeave({ tms2010: tms2010Of(row), operationModeEnum: "Custom" });
    await onQuery(true);
    await showLastProcException();
...
async function onSave() {
  ...
  try {
    await frmMS2120RHFullApi.saveTms2013RHData({ ...bof });
    await loadBof(true);
    toast("保存成功！", 2000, "success");
  } catch {
    /* 拦截层已 toast */
        ...
        <Splitter layout="horizontal" class="h-full min-h-0">
          <SplitterPanel :size="30" :minSize="15" class="flex min-h-0 flex-col overflow-hidden">
            ...
            <div class="flex h-8 shrink-0 items-center gap-2 border-b border-border/60 px-2">
              <span class="text-xs text-muted-foreground"
                >炉次号：{{ focused?.tms2010_cStoveNo ?? focused?.tms2000_cStove ?? "" }}</span
              >
            ...
            <div class="min-h-0 flex-1 overflow-hidden">
              <AgGridVue
                ...
                :locale-text="AG_GRID_LOCALE_CN"
                :default-col-def="hmxDefaultColDef"
                :column-defs="inputCols"
                :row-data="inputRows"
                :pagination="false"
                @grid-ready="onInputReady"
                @first-data-rendered="autoSizeOnFirstData"
            ...
            <div class="h-36 shrink-0 overflow-hidden">
              <AgGridVue
                ...
                :locale-text="AG_GRID_LOCALE_CN"
                :default-col-def="hmxDefaultColDef"
                :column-defs="inputDetailCols"
                :row-data="inputDetailRows"
                :pagination="false"
                @grid-ready="onInputDetailReady"
                @first-data-rendered="autoSizeOnFirstData"
              />
            </div>
          </SplitterPanel>
          ...
          <SplitterPanel :size="45" :minSize="25" class="flex min-h-0 flex-col overflow-hidden">
            <div class="flex h-9 shrink-0 items-center gap-2 border-b border-border/60 px-2">
              <Checkbox v-model="autoRefreshRh" binary input-id="autoRefreshBof" />
