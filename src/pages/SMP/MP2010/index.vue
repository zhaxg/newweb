...
const comOrder = ref('');
const icboReview = ref('');
const txtGz = ref('');
const dtPlan = ref<Date | null>(null);

const colDefs = ref<ColDef[]>(([
      { field: 'NSfpj', headerName: '水封件', width: 150 },
...
function onbtnReview() { /* TODO: 接入业务逻辑 */ }
function onbtnCancelReview() { /* TODO: 接入业务逻辑 */ }
function onbtnUpdateSpec() { /* TODO: 接入业务逻辑 */ }
...
<template>
  <div class="flex min-h-0 flex-1 flex-col">
    ...
    <div class="flex h-9 shrink-0 items-center gap-1 border-b border-border/60 px-2">
        ...
        <label class="whitespace-nowrap">批量订单号</label>
        <InputText v-model="comOrder" class="w-44" />
        <label class="whitespace-nowrap">评审状态</label>
        <Select v-model="icboReview" :options="[]" placeholder="请选择" class="w-40" />
        <label class="whitespace-nowrap">钢种</label>
        <InputText v-model="txtGz" class="w-44" />
    ...
    <div class="flex h-9 shrink-0 items-center gap-1 border-b border-border/60 px-2">
      ...
      <Button label="评审" severity="secondary" @click="onbtnReview" />
      <Button text size="small" class="shrink-0 whitespace-nowrap" @click="onbtnCancelReview"><IconRotate class="h-3.5 w-3.5" />撤销评审</Button>
      <Button text size="small" class="shrink-0 whitespace-nowrap" @click="onbtnUpdateSpec"><IconRefresh class="h-3.5 w-3.5" />更新规格</Button>
      </div>
