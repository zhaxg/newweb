...
async function onPush() {
  ...
  const error1 = lst
    .filter((x) => (x.nStatus ?? x.NStatus) !== OrderStatusEnum.WaitPlan)
    .map((x) => x.cOrderNo ?? x.COrderNo ?? "");
  if (error1.length) {
...
<template>
  <div class="flex min-h-0 flex-1 flex-col">
    ...
    <div class="flex h-9 shrink-0 items-center gap-1 border-b border-border/60 px-2">
      ...
      <Button text class="shrink-0 whitespace-nowrap" @click="onPush"> <IconCheck class="h-3 w-3" />确定提料 </Button>
