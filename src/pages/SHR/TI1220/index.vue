...
/** 对应 FrmTI1220（宽度实绩）：DDH.Winforms.SHR.Forms.InterInfoQuery.FrmTI1220
 *  已接入：tI1220Api.queryTi1220（主表·宽度负差列表）；queryDetail（汇总明细页签）；queryYearDetial（年汇总明细页签）；
 *          systemKeyValueApi.querySysKvItemList("A0000:ZGAUTHOR")（精轧责任者A 下拉，取 cGroup=="JZ"）
 *  待接入：无
 *  偏差：主表按 C# QueryTi1220 返回的 Thr3010Fcdetail 列集渲染；汇总/年汇总为分组表头（≤50mm/50＜毛宽≤80mm/80＜毛宽≤100mm/
 *        100＜毛宽≤120mm/其他/一次命中，每组 支数+命中率），命中率按 C# {0}% 格式化；年汇总时间范围取 [今年1月1日, 明天]
 *        （照 C# AddMonths/AddDays 算式）；责任者下拉候选由 KV 灌值；「抽钢责任者姓名」缩「抽钢责任者」、「生产时间区间」缩「生产时间」统一 w-18。*/
...
interface Ti1220Query extends DtoTi1210Query {
  cZpNo?: string | null;
  cSlabNo?: string | null;
}
...
function buildQuery(): Ti1220Query {
  return {
...
const mainColDefs: ColDef[] = [
  ...
  { field: "nWidthSj", headerName: "实际宽度", minWidth: 88 },
  { field: "nThick", headerName: "订单厚度", minWidth: 88 },
  { field: "nWidth", headerName: "订单宽度", minWidth: 88 },
  { field: "nLen", headerName: "订单长度", minWidth: 88 },
  { field: "nPlanWidthMin", headerName: "目标宽度下限(内控)", minWidth: 150 },
  { field: "nPlanWidthMax", headerName: "目标宽度上限(内控)", minWidth: 150 },
  { field: "nWidthTol", headerName: "宽度公差", minWidth: 88 },
  { field: "cTol", headerName: "公差", minWidth: 88 },
  { field: "cTrimFlag", headerName: "切边方式", minWidth: 88 },
...
const detailGroups: ColDef[] = [
  rateGroup("≤50mm", "nQua1", "hitRate1"),
  rateGroup("50＜毛宽≤80mm", "nQua2", "hitRate2"),
  rateGroup("80＜毛宽≤100mm", "nQua3", "hitRate3"),
  rateGroup("100＜毛宽≤120mm", "nQua4", "hitRate4"),
  rateGroup("其他", "nQua5", "hitRate5"),
  rateGroup("一次命中", "nQua6", "hitRate6"),
];
...
const sumRows = shallowRef<Ti1220Detail[]>([]);
const yearRows = shallowRef<Ti1220Detail[]>([]);
...
async function dataBind() {
  ...
  try {
    if (activeTab.value === 0) {
      sumRows.value = (await tI1220Api.queryDetail(buildQuery())) ?? [];
      requestAnimationFrame(() => sumApi.value?.autoSizeAllColumns());
    } else {
      yearRows.value = (await tI1220Api.queryYearDetial(yearRange())) ?? [];
...
async function onQuery() {
  loading.value = true;
  try {
    mainRows.value = (await tI1220Api.queryTi1220(buildQuery())) ?? [];
    ...
    <Splitter layout="vertical" class="min-h-0 flex-1">
      <SplitterPanel :size="50" :minSize="20" class="flex min-h-0 flex-col overflow-hidden">
        <div class="flex h-8 shrink-0 items-center border-b border-border/60 px-2">
          <span class="text-xs font-medium text-muted-foreground">宽度负差列表</span>
        </div>
