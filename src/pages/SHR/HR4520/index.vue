...
/** 对应 FrmHR4520（当日堆冷台账）：DDH.Winforms.SHR.Forms.FrmHR4520
 *  已接入：hR3600Api.queryHls（上·堆冷明细）、hR3600Api.queryHR4510ToDays（下·台账信息汇总）；
 *          两表查询固定堆冷状态=已入缓冷区（原 Thr3010HlStatusEnum.EnterFur），无查询条件控件
 *  待接入：无（原窗体无二级弹窗、无输入控件）
...
/* 原 DataBind/HzDataBind 固定条件：仅缓冷状态=已入缓冷区 */
function buildDto(): DtoQuerySlabs {
  return { nHlStatus: Thr3010HlStatusEnum.EnterFur };
}
...
/* ---------- 汇总表 bandedGridView1 / HR4510HzDto（20 可见列 + 隐藏日期，列头取实体 LDisplay） ---------- */
const hzColDefs: ColDef[] = [
  ...
  { colId: "nSlabDlHqQua", field: "nSlabDlHqQua", headerName: "去火切数量", width: 112 },
  { colId: "nSlabDlHqWgt", field: "nSlabDlHqWgt", headerName: "去火切重量", width: 112 },
  { colId: "nSlabDlQua", field: "nSlabDlQua", headerName: "堆冷坑支数", width: 112 },
  { colId: "nSlabDlWgt", field: "nSlabDlWgt", headerName: "堆冷坑重量", width: 112 },
...
async function query() {
  loading.value = true;
  try {
    rows.value = (await hR3600Api.queryHls(buildDto())) ?? [];
...
async function queryHz() {
  ...
  try {
    hzRows.value = (await hR3600Api.queryHR4510ToDays(buildDto())) ?? [];
...
<template>
  <div class="flex min-h-0 flex-1 flex-col">
    <!-- 工具栏（原 btnQuery 唯一按钮；标题为原 groupControl1「堆冷明细」。原窗体无查询条件控件） -->
