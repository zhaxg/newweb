...
function calcRates() {
  ...
  for (const x of rows.value) {
    s.r[0] += x.nWgtCp ?? 0; s.r[1] += x.nWgt ?? 0;
    s.r4[0] += x.nWgtCp4 ?? 0; s.r4[1] += x.nWgt4 ?? 0;
    s.r2[0] += x.nWgtCp2 ?? 0; s.r2[1] += x.nWgt2 ?? 0;
  }
  const pct = (p: [number, number]) => (p[0] === 0 || p[1] === 0 ? "0" : (Math.round((p[0] * 100 / p[1]) * 100) / 100).toFixed(2));
