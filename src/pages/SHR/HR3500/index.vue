...
/** 对应 FrmHR3500（定尺剪作业 / 切头剪作业 HR3300 共用）：DDH.Winforms.SHR.Forms.FrmHR3500
 *  已接入：hR3400Api.queryList / hR3000Api.queryThr3030s
 *  待接入：二级弹窗 FrmHR3510（定尺剪剪切，isQt=1）/ FrmHR3520（切头剪剪切，isQt≠1）/ FrmHR3020（剪切计划调整）
 *  菜单参数：cQueryString "ZG01,2"=定尺剪（HR3500/HR3510 弹窗）、"ZG01,1"=切头剪（HR3300/FrmHR3520 弹窗），parts[1] 即原 isQt */
...
/* ---------- 上表：待剪切材料明细（gridView1 / Thr3040，勾选=Selected） ---------- */
...
function onGrid1SelectionChanged() {
  ...
  void dataBind3030(current1.value);
}
...
const colDefs2: ColDef[] = [
  ...
  { colId: "cMxId", field: "cMxId", headerName: "THR3010主键", width: 150, hide: true },
  { colId: "cLineCode", field: "cLineCode", headerName: "产线代码", width: 112, hide: true },
  { colId: "cSpec", field: "cSpec", headerName: "订单规格", width: 112, hide: true },
  { colId: "selected", field: "selected", headerName: "选择", width: 112, hide: true },
];
...
function onAdd() {
  ...
  if (!row) return;
  const name = isQt === "1" ? "FrmHR3510" : "FrmHR3520";
  // 原剪切弹窗：isQt=1 → FrmHR3510（定尺剪），否则 FrmHR3520（切头剪）；OK 后回刷待剪切材料明细
  toast(`剪切弹窗 ${name} 待接入`, 2500, "warn");
}

function onChange() {
  const sel = (grid1Api.value?.getSelectedRows() ?? []) as Thr3040[];
  if (sel.length === 0) return;
  // 原 FrmHR3020：剪切计划调整弹窗（对所选材料改剪切计划），OK 后回刷，待接入
  toast("剪切计划调整弹窗 FrmHR3020 待接入", 2500, "warn");
}
</script>
...
<template>
  <div class="flex min-h-0 flex-1 flex-col">
    ...
    <div class="flex h-9 shrink-0 items-center gap-1 border-b border-border/60 px-2">
      ...
      <Button variant="outlined" class="shrink-0 whitespace-nowrap" @click="onAdd">剪切</Button>
      <Button variant="outlined" class="shrink-0 whitespace-nowrap" @click="onChange">剪切计划调整</Button>
    </div>

    <!-- 上下主子表：原 splitContainerControl1 612/983 ≈ 62% -->
