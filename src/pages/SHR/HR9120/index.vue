...
/** 对应 FrmHR9120（钢板取样实绩）：DDH.Winforms.SHR.Forms.FrmHR9120
 *  已接入：hR4200Api.queryTiP48j04s
 *  偏差：无 */
...
import { hR4200Api, type DtoQueryL2, type TiP48j04, type TimeRange } from "@/api/mes4ddh/shr.swagger";
...
/* ---------- 表格（gridView1 / TiP48j04） ---------- */
const rows = shallowRef<TiP48j04[]>([]);
...
async function query() {
  ...
  try {
    ...
    rows.value = (await hR4200Api.queryTiP48j04s(dto)) ?? [];
