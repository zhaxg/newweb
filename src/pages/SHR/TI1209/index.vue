 ...
 *  已接入：tI1209Api.getListAsync（关键字 + 时间区间；默认 [今天-6天, 今天23:59:59]，原 Today.AddDays(-6) ~ Today.AddDays(1).AddSeconds(-1)）
 *  待接入：编辑弹窗 FrmTI1209_Edit（原窗体内 ShowDialog 二级窗体，保存走 ITI1209AppService.UpdateAsync，
 *         swagger 未生成 tI1209Api.updateAsync；编辑按钮仅保留原「未选行提示」校验后占位）
...
import { tI1209Api, type TiL2me09, type TimeRange } from "@/api/mes4ddh/shr.swagger";

const theme = makeHmxGridTheme();
const { toast } = useToast();
const rows = ref<TiL2me09[]>([]);
...
/* ---------- 表格（gridView1 / TiL2me09，Designer 可见 19 列 + 隐藏 2 列） ---------- */
...
async function onQuery() {
  querying.value = true;
  try {
    rows.value = (await tI1209Api.getListAsync(keyword.value.trim() || undefined, toTimeRange(dates.value))) ?? [];
...
/* 原 btnEdit_Click：无焦点行 →「请选择信息」；有则开 FrmTI1209_Edit（待接入占位） */
function onEdit() {
  const r = gridApi.value?.getSelectedRows()[0] as TiL2me09 | undefined;
  if (!r) {
    toast("请选择信息", 2000, "warn");
    return;
  }
  toast("编辑弹窗（FrmTI1209_Edit）待接入", 2000, "warn");
}
</script>
