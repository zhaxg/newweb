...
/** 对应 FrmHR5300_JC（棒材轧制作业）：DDH.Winforms.SHR.Forms.ThrBar.FrmHR5300_JC
 *  已接入：tPa1000Api.queryLines / hR5300JCZFApi.getListAsync1 / getListAsync2 / rollOutAsync / finishRoll / canleFinishRoll / hR5200JCTLApi.canleHR5200
 *  待接入：无
 *  偏差：轧废/轧制完成动作原用固定 QueryString 产线，web 侧随下拉当前值提交（产线默认值取菜单 cQueryString） */
...
const reason = ref("");
const nQuaZF = ref<number>(1);

/* ---------- 上表：可轧废材料（gridView1 / QueryHR5200_JCDto） ---------- */
...
/* ---------- 下表：轧废记录（gridView2 / Thr5200，Selected 勾选=行选择） ---------- */
...
const topColDefs: ColDef[] = [
  ...
  { colId: "nQuaZf", field: "nQuaZf", headerName: "轧废支数", width: 112 },
  { colId: "nWgtZf", field: "nWgtZf", headerName: "轧废重量", width: 112 },
  { colId: "nQuaTL", field: "nQuaTL", headerName: "剔炉支数", width: 112, hide: true },
  { colId: "nWgtTL", field: "nWgtTL", headerName: "剔炉重量", width: 112, hide: true },
];
...
/* ---------- 轧制完成 / 撤销轧制完成 / 轧废 / 撤销轧废 ---------- */
async function doFinishRoll() {
  const cur = topCurrent.value;
  if (!cur) return;
  try {
    await hR5300JCZFApi.finishRoll({
      cLineCode: lineCode.value,
      nQuaZf: nQuaZF.value ?? 0,
      cStoveNo: cur.cStove,
      cSgCode: cur.cSgCode,
      cSpec: cur.cSpec,
      nQua: cur.nQua ?? 0,
...
function btnRollFinish() {
  const cur = topCurrent.value;
  if (!cur) return;
  const n = nQuaZF.value ?? 0;
  if (n <= 0 || n > (cur.nQua ?? 0)) {
    toast("轧制完成支数必须大于0且小于该炉剩余支数！", 2000, "warn");
    return;
  }
  askConfirm(`确认炉号：${cur.cStove} 轧制完成${n} 条数据？`, doFinishRoll);
}

async function doCanleFinishRoll() {
  const cur = topCurrent.value;
  if (!cur) return;
  try {
    await hR5300JCZFApi.canleFinishRoll({
      cLineCode: lineCode.value,
      nQuaZf: nQuaZF.value ?? 0,
      cStoveNo: cur.cStove,
      cSgCode: cur.cSgCode,
...
function btnCanleRollFinish() {
  const cur = topCurrent.value;
  if (!cur) return;
  const n = nQuaZF.value ?? 0;
  if (n <= 0 || n > (cur.nFinishQua ?? 0)) {
    toast("撤销轧制完成支数必须大于0且小于该炉完成支数！", 2000, "warn");
    return;
  }
  askConfirm(`确认撤销炉号：${cur.cStove} 轧制完成${n} 条数据？`, doCanleFinishRoll);
}
...
async function doRollOut() {
  ...
  if (!cur) return;
  try {
    await hR5300JCZFApi.rollOutAsync({
      cLineCode: lineCode.value,
      nQuaZf: nQuaZF.value ?? 0,
      ...
      cSpec: cur.cSpec,
      nQua: cur.nQua ?? 0,
      cOrderNo: cur.cOrderNo,
    });
    toast("轧废成功！", 2000, "success");
    await dataBind();
...
function btnZF() {
  ...
  askConfirm(`确认炉号：${cur.cStove} 轧废${n} 条数据？`, doRollOut);
}

async function doCanleZF() {
  const sel = (bottomApi.value?.getSelectedRows() ?? []) as Thr5200[];
  const pieceNos = sel.map((x) => x.cPieceNo).filter((x): x is string => !!x);
  try {
    await hR5200JCTLApi.canleHR5200(ThrLogJcEnum.Zf, pieceNos);
    await dataBind();
    await dataBindThrLogJc();
  } catch {
    /* 拦截层已 toast */
...
function btnCanle() {
  ...
  askConfirm("是否确定撤销选择数据的剔炉记录？", doCanleZF);
}
