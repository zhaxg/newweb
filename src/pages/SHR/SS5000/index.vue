...
onMounted(async () => {
  try {
    for (const x of (await systemKeyValueApi.getSysKvListByGroup("A0000:SHIFT")) ?? []) kvShift.set(x.cCode ?? "", x.cName ?? "");
  } catch {
    /* 拦截层已 toast */
  }
  try {
    for (const x of (await systemKeyValueApi.getSysKvListByGroup("A0000:DG_GROUP")) ?? []) kvGroup.set(x.cCode ?? "", x.cName ?? "");
  } catch {
