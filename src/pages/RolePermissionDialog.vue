...
const parentOf = computed(() => {
  ...
  const walk = (list: ErpMenuNode[], pid: string | null) => {
...
function toPTree(list: ErpMenuNode[]): PTreeNode[] {
  return list.map((n) => {
...
const treeNodes = computed<PTreeNode[]>(() => toPTree(erpMenu));
...
function setNodeCheckedById(id: string, nextChecked: boolean) {
  ...
  const apply = (n: ErpMenuNode) => {
    if (nextChecked) next.add(n.id);
    else next.delete(n.id);
    n.children?.forEach(apply);
  };
  const find = (list: ErpMenuNode[]): ErpMenuNode | null => {
    for (const n of list) {
      if (n.id === id) return n;
      if (n.children?.length) {
