/*
 * ERP Chrome 风格标签栏元素（vendored 自 qn-chrome-tabs@1.1.0，MIT）
 * https://github.com/qlynick/qn-chrome-tabs
 *
 * 本地改动：
 * - 移除分组、新建标签按钮、editAddress、图标/提示渲染钩子（ERP 壳层不使用）
 ...
 */
import styles from "./erpChromeTabs.css?inline";

export const ERP_CHROME_TABS_TAG = "erp-chrome-tabs";

export const erpChromeTabsEvents = {
...
export interface ErpChromeTabItem {
  id: string;
  title: string;
...
export interface ErpChromeTabEventDetail {
  tabId: string;
}

export interface ErpChromeTabReorderEventDetail {
  tabId: string;
  targetTabId: string;
  position: "before" | "after";
}

export type ErpChromeTabsLocale = "zh" | "en";
export type ErpChromeTabTooltipMode = "truncated" | "always" | "never";
export type ErpChromeTabsWidthMode = "fixed" | "content";
...
function browserLocale(): ErpChromeTabsLocale {
  return navigator.language.toLowerCase().startsWith("en") ? "en" : "zh";
}

export class ErpChromeTabsElement extends HTMLElement {
  #tabs: ErpChromeTabItem[] = [];
  #activeTabId = "";
  #locale: ErpChromeTabsLocale = browserLocale();
  #tabTooltipMode: ErpChromeTabTooltipMode = "truncated";
  ...
  set tabs(value: ErpChromeTabItem[]) {
  ...
  set locale(value: ErpChromeTabsLocale) {
    this.#locale = value in messages ? value : "zh";
    this.#render();
  }

  get locale(): ErpChromeTabsLocale {
    return this.#locale;
  }

  set tabTooltipMode(value: ErpChromeTabTooltipMode) {
    this.#tabTooltipMode = value === "always" || value === "never" ? value : "truncated";
    if (this.#tabTooltipMode === "never") this.#closeTabPopover();
  }

  get tabTooltipMode(): ErpChromeTabTooltipMode {
    return this.#tabTooltipMode;
  }

  set widthMode(value: ErpChromeTabsWidthMode) {
    if (value === "content") {
  ...
  get widthMode(): ErpChromeTabsWidthMode {
    return this.getAttribute("data-width-mode") === "content" ? "content" : "fixed";
  }

  #emit(name: string, detail?: ErpChromeTabEventDetail | ErpChromeTabReorderEventDetail) {
  ...
  #render() {
    ...
    for (const tab of this.#tabs) {
      ...
      element.addEventListener("click", () => {
        ...
        this.#emit(erpChromeTabsEvents.activate, { tabId: tab.id });
      });
      ...
      element.addEventListener("drop", (event) => {
        ...
        this.#emit(erpChromeTabsEvents.reorder, { tabId, targetTabId: tab.id, position });
      });
      ...
      close.addEventListener("click", (event) => {
        ...
        const finish = () => {
          if (emitted) return;
          emitted = true;
          this.#emit(erpChromeTabsEvents.close, { tabId: tab.id });
        };
  ...
  #showContextMenu(event: MouseEvent, tabId: string) {
    ...
    for (const [label, eventName] of [
      [text.close, erpChromeTabsEvents.close],
      [text.closeRight, erpChromeTabsEvents.closeRight],
      [text.closeOthers, erpChromeTabsEvents.closeOthers],
      [text.closeLeft, erpChromeTabsEvents.closeLeft],
    ] as const) {
      ...
      if (eventName === erpChromeTabsEvents.close) {
...
if (!customElements.get(ERP_CHROME_TABS_TAG)) {
  customElements.define(ERP_CHROME_TABS_TAG, ErpChromeTabsElement);
}

declare global {
  interface HTMLElementTagNameMap {
    [ERP_CHROME_TABS_TAG]: ErpChromeTabsElement;
  }
}
