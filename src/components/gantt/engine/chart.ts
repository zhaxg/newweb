...
import { assertNoActualTime, moveSelectedHorizontally, moveSelectedVertically } from "./rules";
import {
  drawCrosshair,
  drawGridLines,
  drawLeftColumn,
...
export class GanttChart {
    ...
    if (this.scrollMetrics.vVisible) {
      ...
      if (p.x >= size.width - bar && p.y >= track.y && p.y < track.y + track.length) {
        ...
        if (kind === "vtrack" && thumbs.v) {
          this.scrollY = clamp(
            this.scrollY + (p.y < thumbs.v.y ? -this.scrollMetrics.vLargeChange : this.scrollMetrics.vLargeChange),
            0,
            this.scrollMetrics.vMax,
          );
        }
    ...
    if (this.scrollMetrics.hVisible) {
      ...
      if (p.y >= size.height - bar && p.x >= track.x && p.x < track.x + track.length) {
        ...
        if (kind === "htrack" && thumbs.h) {
          this.scrollX = clamp(
            this.scrollX + (p.x < thumbs.h.x ? -this.scrollMetrics.hLargeChange : this.scrollMetrics.hLargeChange),
            0,
            this.scrollMetrics.hMax,
          );
        }
        this.drag = { kind, startX: p.x, startY: p.y, startScrollX: this.scrollX, startScrollY: this.scrollY };
        return true;
  ...
  private applyScrollbarDrag(p: { x: number; y: number }): void {
    if (!this.drag) return;
    const size = this.viewport();
    const thumbs = scrollbarThumbs(
      this.scrollMetrics,
      this.style,
      size,
      this.drag.startScrollX,
      this.drag.startScrollY,
    );
    ...
    if (this.drag.kind === "vthumb" && thumbs.v) {
      ...
      this.scrollY = clamp(
        this.drag.startScrollY + (dy / travel) * this.scrollMetrics.vMax,
        0,
        this.scrollMetrics.vMax,
      );
      ...
      this.scrollX = clamp(
        this.drag.startScrollX + (dx / travel) * this.scrollMetrics.hMax,
        0,
        this.scrollMetrics.hMax,
      );
    }
