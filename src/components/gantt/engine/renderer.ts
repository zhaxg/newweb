...
function strokeRect(ctx: CanvasRenderingContext2D, r: Rect, color: string, scale: PixelScale, deviceWidth = 1): void {
...
function hourLabel(v: LayoutView, hourIndex: number): string {
  const midnight = new Date(v.startDate.getFullYear(), v.startDate.getMonth(), v.startDate.getDate());
...
export function drawLeftColumn(ctx: CanvasRenderingContext2D, v: RenderModel, size: Viewport): void {
  ...
  drawTextCenter(ctx, headerCaption(v), header, s.resourceCaptionHeaderFont, s.resourceCaptionHeaderFontColor, scale);
...
export function scrollbarThumbs(
  ...
  if (metrics.vVisible) {
    ...
    const thumbH = Math.max(
      24,
      (track.length * metrics.vLargeChange) / Math.max(1, metrics.vMax + metrics.vLargeChange),
    );
  ...
  if (metrics.hVisible) {
    ...
    const thumbW = Math.max(
      24,
      (track.length * metrics.hLargeChange) / Math.max(1, metrics.hMax + metrics.hLargeChange),
    );
...
export function resourceRowAt(v: RenderModel, size: Viewport, clientY: number): ResourceObject | null {
