 ...
 */
export function computeScrollMetrics(v: LayoutView, viewportWidth: number, viewportHeight: number): ScrollMetrics {
  const s = v.style;
  ...
  const vMaximum = totalCaptionHeight + vLargeChange - bodyHeight + (hVisible ? s.scrollbarSize : 0);
  const hMaximum =
    Math.trunc(totalHours(v)) * s.ruleHourLength + hLargeChange - bodyWidth + (vVisible ? s.scrollbarSize : 0);
