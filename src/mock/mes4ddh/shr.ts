...
const q4 = (base: string, make: (i: number) => Record<string, unknown>): RouteMap =>
  Object.fromEntries(
    [
      ...
      "queryTiP48j07s",
      "queryTiP48j09s",
    ].map((m) => [`post ${base}/${m}`, (config: Parameters<typeof ok>[0]) => ok(config, demoRows(3, make))]),
  );
...
export const shrRoutes: RouteMap = {
  ...
  [`post ${S}/hR3000/queryThr3000s`]: (config) =>
    ok(
      config,
      demoRows(3, (i) => ({ ...row(i), cLineCode: "ZG01", cStove: "S260901" })),
    ),
  [`post ${S}/hR3000/queryThr3010s`]: (config) =>
    ok(
      config,
      demoRows(3, (i) => ({ ...row(i), cPlateNo: `P${i + 1}`, cPrintCode: `PR${i + 1}` })),
    ),

  /* ---------- hR9000（HR9xxx 实绩/台账/报表） ---------- */
  [`post ${S}/hR9000/querySlabs`]: (config) =>
    ok(
      config,
      demoRows(3, (i) => ({ ...row(i), cStove: "S260901" })),
    ),
  ...
  [`post ${S}/hR9000/queryTiL2me01s`]: (config) =>
    ok(
      config,
      demoRows(3, (i) => ({ ...row(i), zone: "FUR", reason: 1 })),
    ),
  ...
  [`post ${S}/hR4000/queryPrintSjs`]: (config) =>
    ok(
      config,
      demoRows(3, (i) => ({ ...row(i), cSlCode: "CS1", cPrint: "1", cStoreCode: "ZG01-01" })),
    ),
  [`post ${S}/hR4000/querySg`]: (config) =>
    ok(
      config,
      demoRows(3, (i) => ({ id: `SG${i + 1}`, cSgSign: `Q235B${i}` })),
    ),
  ...
  [`post ${S}/hR4300/queryCc`]: (config) =>
    ok(
      config,
      demoRows(3, (i) => ({ ...row(i), cSlCode: "CS1", cTrimFlag: "01" })),
    ),
  ...
  [`post ${S}/hR4600/queryRsl`]: (config) =>
    ok(
      config,
      demoRows(3, (i) => ({ date: `2026-09-0${i + 1}`, nQuaTotal: 120, nQua: 100, nRsl: 83.33 })),
    ),
  [`post ${S}/hR4600/queryRzl`]: (config) =>
    ok(
      config,
      demoRows(3, (i) => ({ date: `2026-09-0${i + 1}`, group: `G${i + 1}`, nQuaTotal: 120, nQua500: 60, nQua600: 60 })),
    ),
  ...
  [`post ${S}/hR4700/queryThickHz`]: (config) =>
    ok(
      config,
      demoRows(3, (i) => ({ ...row(i), cSlabNo: `DEMO-SLAB-${i + 1}` })),
    ),
  ...
  [`post ${S}/hR4810/get4810CclHzDtos`]: (config) =>
    ok(
      config,
      demoRows(3, (i) => ({ cDate: `2026-09-0${i + 1}`, nQua: 20, nWgt: 2000, nRate: 95 })),
    ),
  [`post ${S}/hR4810/get4810CclSgCodeHzDtos`]: (config) =>
    ok(
      config,
      demoRows(3, (i) => ({ cSgCode: `Q235${i}`, nQua: 20, nWgt: 2000, nRate: 95 })),
    ),
  [`post ${S}/hR4810/get4810CclThickHzDtos`]: (config) =>
    ok(
      config,
      demoRows(3, (i) => ({ nThick: 10 + i, nQua: 20, nWgt: 2000, nRate: 95 })),
    ),
  [`post ${S}/hR4810/get4810CclGroupHzDtos`]: (config) =>
    ok(
      config,
      demoRows(3, (i) => ({ group: `G${i + 1}`, nQua: 20, nWgt: 2000, nRate: 95 })),
    ),
  [`post ${S}/hR4810/get4810CclDayHzDtos`]: (config) =>
    ok(
      config,
      demoRows(3, (i) => ({ cDate: `2026-09-0${i + 1}`, nQua: 20, nWgt: 2000, nRate: 95 })),
    ),
  ...
  [`post ${S}/hR4900/get4900Dtos`]: (config) =>
    ok(
      config,
      demoRows(3, (i) => ({ ...row(i) })),
    ),
  ...
  ...Object.fromEntries(
    [
      ...
      "get9300HzFmEnd",
      "get9300HzRmGroupHz",
      "get9300HzFmStGroupHz",
      "get9300HzFmEndGroupHz",
      "get9300HzInFurTimeGroupHz",
      "get9300HzFinishTempGroupHz",
      "get9300HzFinishTempAuthorHz",
      "get9300HzEntryTempGroupHz",
      "get9300HzEntryTempAuthorHz",
      "get9300HzFmTempAuthorHz",
      "get9300HzFmTempGroupHz",
      "get9300HzRollTempHz",
      "get9300HzACCTempGroupHz",
      "get9300HzACCTempAuthorHz",
    ].map((m) => [
      `post ${S}/hR9300/${m}`,
      (config: Parameters<typeof ok>[0]) =>
        ok(
          config,
          demoRows(3, (i) => ({ cShiftGroup: `G${i + 1}`, author: `U00${i + 1}`, nQua: 8 + i, nPass: 5, nFail: 1 })),
        ),
    ]),
  ),
  ...
  [`post ${S}/hR9320/get9320Dtos`]: (config) =>
    ok(
      config,
      demoRows(3, (i) => ({ ...row(i), cTestNo: `T26090${i + 1}` })),
    ),
  [`post ${S}/hR9400/getHR9400Dtos`]: (config) =>
    ok(
      config,
      demoRows(3, (i) => ({ ...row(i), cBatchOrder: `B26090${i + 1}-1` })),
    ),
  [`post ${S}/hR9400/getHR9410Dtos`]: (config) =>
    ok(
      config,
      demoRows(3, (i) => ({ ...row(i) })),
    ),
  ...
  [`post ${S}/hR9500/queryOrderKc`]: (config) =>
    ok(
      ...
      demoRows(3, (i) => ({
        ...row(i),
        nProType: i === 0 ? 0 : i === 1 ? 10 : 20,
        cStoreCode: i === 1 ? "ZG01-04" : "ZG01-01",
        nStatus: i === 2 ? 99 : 1,
      })),
    ),
  ...
  ["post /dDH.Service.SHR.Services.InterInfoQuery/tI1010/queryTi1010"]: (config) =>
    ok(
      config,
      demoRows(3, (i) => ({ ...row(i), dHeatTime: "2026-09-20 06:00:00", nTempIn: 1180 + i })),
    ),
